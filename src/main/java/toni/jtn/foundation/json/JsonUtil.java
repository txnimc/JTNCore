package toni.jtn.foundation.json;
...
public class JsonUtil {
    ...
    /**
     * Checks if an items is empty, and if it is, returns false and logs the key.
     */
    public static boolean checkAndLogEmpty(JsonElement e, ResourceLocation id, String type, Logger logger) {
        String s = e.toString();
        if (s.isEmpty() || "{}".equals(s)) {
            logger.error("Ignoring {} items with id {} as it is empty.  Please switch to a condition-false json instead of an empty one.", type, id);
            return false;
        }
        return true;
    }
...
//     * @return True if the items's conditions are met, false otherwise.
//     */
...
//            return true;
//        }
//        logger.trace("Skipping loading {} items with id {} as it's conditions were not met", type, id);
//        return false;
//    }
